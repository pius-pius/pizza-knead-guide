...
const FlourMixer = ({ targetWeight, flours, onFloursChange, inline, maturationHours }: FlourMixerProps) => {
  ...
  const addFlour = () => {
    ...
    onFloursChange([...adjusted, { id: nextId++, name: `Farina ${flours.length + 1}`, w: 250, percent: newPercent }]);
  };
  ...
  return (
    <div className={inline ? "space-y-3" : "px-4 py-6 space-y-4"}>
      {!inline && <h2 className="text-2xl font-bold text-center mb-2">Miscela Farine</h2>}
          ...
          <div key={flour.id} className="bg-secondary/50 rounded-xl p-3">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                <p className="text-[10px] text-muted-foreground uppercase">Percentuale</p>
      ...
      {flours.length < 4 && (
        <Button variant="outline" onClick={addFlour} className="w-full rounded-xl">
          <Plus className="h-4 w-4 mr-2" /> Aggiungi farina
        </Button>
      )}
      ...
      {wTarget && (
        <div className={`rounded-xl p-3 border-2 ${
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] font-semibold text-muted-foreground uppercase mb-0.5">W ottenuto</p>
            ...
            <div className="text-right">
              <p className="text-[10px] font-semibold text-muted-foreground uppercase mb-0.5">Target</p>
          ...
          <div className="mt-2">
            ...
            <p className="text-[10px] mt-1 font-semibold text-center" style={{
              ...
              {Math.abs(mixW - wTarget) <= 15
                ? "✅ W nel range ideale!"
                : mixW < wTarget
                  ? `↓ W troppo basso (−${wTarget - mixW})`
                  : `↑ W troppo alto (+${mixW - wTarget})`}
            </p>
          </div>
        </div>
      )}
      ...
      <div className={`grid ${wTarget ? "grid-cols-1" : "grid-cols-2"} gap-2`}>
        ...
        {targetWeight && (
          <div className={`rounded-xl p-3 text-center border ${
            ...
            <p className="text-[10px] font-semibold text-muted-foreground uppercase mb-0.5">Peso totale</p>
            <p className="text-2xl font-display font-bold" style={{
              color: Math.abs(totalWeight - targetWeight) <= 5 ? "hsl(142, 71%, 35%)" : "hsl(32, 95%, 44%)"
            }}>{totalWeight}g</p>
            <p className="text-[9px] text-muted-foreground">target: {targetWeight}g</p>
          </div>
        )}
      </div>
    </div>
